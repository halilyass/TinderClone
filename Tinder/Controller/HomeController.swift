...
class HomeController: UIViewController {
    ...
    private var viewModels = [CardViewModal]() {
    ...
    func fetchUsers(forCurrentUser user: User) {
        Service.fetchUsers(forCurrentUser: user) { users in
            self.viewModels = users.map({ CardViewModal(user: $0) })
        }
    }
    ...
    func configureCards() {
        viewModels.forEach { viewModel in
            let cardView = CardView(viewModal: viewModel)
...
//            cardViews.append(cardView)
    ...
    func performSwipeAnimation(shouldLike: Bool) {
        ...
        UIView.animate(withDuration: 1.0, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0.1, options: .curveEaseOut) {
            self.topCardView?.frame = CGRect(x: translation,
                                             y: 0, width: (self.topCardView?.frame.width)!,
...
extension HomeController: CardViewDelegate {
    func cardView(_ view: CardView, didLikeUser: Bool) {
        ...
        guard let user = topCardView?.viewModal.user else { return }
...
extension HomeController: BottomControlsStackViewDelegate {
    func handleLike() {
        ...
        saveSwipeAndCheckForMatch(forUser: topCard.viewModal.user, didLike: true)
    }
    ...
    func handleDislike() {
        ...
        Service.saveSwipe(forUser: topCard.viewModal.user, isLike: false, completion: nil)
    }
